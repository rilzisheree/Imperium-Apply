--[[
        ...
        Architecture note:
          CommandRemotes requires WaitForChild on 5 RemoteEvents (15 s timeout each).
          To avoid blocking the input handler for up to 75 s, ALL UI construction and
          the UserInputService binding happen synchronously at script start.
          The remote/registry require happens inside task.spawn so it never delays input.
--]]
...
-- These are populated asynchronously — guarded before use
...
local CFG = {
        ...
        PROMPT_COLOR     = Color3.fromRGB(190, 190, 200),
        CMD_COLOR        = Color3.fromRGB(210, 210, 220),
...
-- ─── Player suggestion helpers ─────────────────────────────────────────────────

local filteredPlayers   = {}
local playerSuggestIndex = 1
local psRows            = {}

local PLAYER_ARG_LABELS = { player = true, from = true, to = true }

local function isPlayerArg(label)
        return PLAYER_ARG_LABELS[label:lower():gsub("%?", "")] == true
end

local function buildPsRow(index)
...
local function highlightPsRow(targetIndex)
...
local function refreshPlayerSuggestions(filter)
        ...
        for _, row in psRows do row.Visible = false end
        ...
        for i = 1, count do
                ...
                if nl then
                        nl.Text       = matched[i]
                        nl.TextColor3 = sel and CFG.PROMPT_COLOR or CFG.PS_TEXT_COLOR
                end
...
local function clearDropdown()
        for _, row in acRows do row.Visible = false end
end

local function buildDropdownRow(index)
        if acRows[index] then return acRows[index] end
...
local function refreshDropdown()
        ...
        for i = 1, count do
                ...
                if descL then
                        -- m.description comes directly from getMatches (no .entry wrapper)
                        descL.Text = m.description or ""
                end
        ...
        if selected and selected.args and #selected.args > 0 then
                local parts = {}
                table.insert(parts, '<font color="#c8c8d8">' .. selected.name .. "</font>")
                for _, arg in selected.args do
                        local isOpt  = arg:sub(-1) == "?"
                        local label  = isOpt and arg:sub(1, -2) or arg
                        local color  = isOpt and "#606070" or "#808090"
                        local open   = isOpt and "[" or "<"
                        local close  = isOpt and "]" or ">"
                        table.insert(parts, '<font color="' .. color .. '">' .. open .. label .. close .. "</font>")
                end
...
local function updateAutocomplete()
        ...
        if hasSpace then
                ...
                if chosen then
                        acMatches = { { name = query:lower(), description = chosen.description, args = chosen.args, permission = chosen.permission } }
                        acIndex   = 1
                else
                        acMatches = {}
                end
...
local function executeCommand()
        ...
        if not CommandRegistry then
                showNotification("⚠  Still loading, try again")
                return
        end
        ...
        if history[1] ~= raw then
                ...
                if #history > CFG.HISTORY_MAX then table.remove(history, #history) end
        end
        ...
        if CommandRemotes and CommandRemotes.CommandExecuted then
                ...
                showNotification("⚠  Remotes not ready, try again")
        end

        closeBar()
end

-- ─── TextBox event wiring ──────────────────────────────────────────────────────
...
-- ─── Input handler ─────────────────────────────────────────────────────────────
--
-- Registered immediately at script start — BEFORE the async remote load below —
-- so the bar responds to ; the instant the script runs, not after a 75-second wait.

UserInputService.InputBegan:Connect(function(input, gameProcessed)

        -- Open: ; while nothing else has keyboard focus
        ...
        -- Escape always closes regardless of TextBox focus state
        ...
        if input.KeyCode == Enum.KeyCode.Tab then
                ...
                elseif #acMatches > 0 then
                        local m = acMatches[acIndex] or acMatches[1]
                        if m then
                                inputBox.Text = m.name .. " "
                                task.defer(function() inputBox.CursorPosition = #inputBox.Text + 1 end)
                        end
...
-- ─── Load remotes asynchronously ───────────────────────────────────────────────
--
-- This runs in a background task so it NEVER blocks the input handler above.
-- The bar opens and closes immediately; commands simply show a "loading" notice
-- if fired before this completes (unlikely — usually done in < 1 s on a live server).
...
task.spawn(function()
        ...
        if not ok then
                warn("[CommandBar] Failed to load remotes/registry:", result)
                return
        end

        -- Wire up server feedback toasts once remotes are available
